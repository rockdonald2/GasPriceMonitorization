...
do
    ...
    then
        echo "Failed to execute upload crude notebook at $(date)." >> $ROOT/error
        exit 1
    fi

    rm $ROOT/scrape.nbconvert.ipynb $ROOT/upload_gas.nbconvert.ipynb $ROOT/upload_crude.nbconvert.ipynb

    echo "Finished scraping GasPrice and CrudeOil data at $(date)." >> $ROOT/log

    echo "Next scrape for GasPrice and CrudeOil in a day." >> $ROOT/log
    sleep 1d
done
