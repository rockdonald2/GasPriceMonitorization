...
do
    ...
    then
        echo "Failed to execute scrape notebook at $(date)." >> /home/jovyan/work/workspace/error
        exit 1
    fi

    if ! jupyter nbconvert --execute /home/jovyan/work/workspace/upload_gas.ipynb --to notebook --ExecutePreprocessor.timeout=-1
    then
        echo "Failed to execute upload gas notebook at $(date)." >> /home/jovyan/work/workspace/error
        exit 1
    fi
